#property copyright "IndexBreakout"
#property version "1.00"
#property strict
#property description "Index adaptation of the corrected HTFTrendBreakout v1.01 rig (range breakout, ATR stops/trail, 0.25% risk). Adds ATR-relative entry air-gap guard and optional server-hour session gate."
...
input ulong InpMagic=5090888;
input int InpDeviationPoints=10;
input double InpMaxSpreadStopFraction=0.35;
input double InpMaxEntryGapPips=15.0;
input int InpMaxRetries=5;
input group "Index adaptation"
input double InpMaxEntryAirGapFrac=1.0;  // entry may be up to this x ATR away from signal close; 0 = legacy pips guard
input int InpSessionStart=0;             // server hour start (inclusive); start==end disables gate
input int InpSessionEnd=0;               // server hour end (exclusive); 0/24 disables gate
...
int g_signals=0,g_entries=0,g_closes=0,g_rejects=0,g_retries=0,g_trailMods=0,g_minlotRejects=0,g_outOfSession=0;
...
{
   if(InpSessionStart>=0 && InpSessionEnd>InpSessionStart && InpSessionEnd<=24)
   {
...
void SignalStep()
{
// Called once when a new signal bar closes. Bar 1 is the just closed signal bar;
...
// Session gate: new longs/shorts only from signals closed inside the liquidity window.
   if(dir!=0 && !InSession(r[0].time))
   {
      g_outOfSession++;
      g_status=StringFormat("Signal %s outside session, skipped (server hour %d)",
         dir>0?"LONG":"SHORT",(int)((r[0].time%86400)/3600));
      return;
   }
   if(dir!=0)
   {
      ...
      g_status=StringFormat("Signal %s %s close %.*f chHi %.5f chLo %.5f ATR %.5f",
         dir>0?"LONG":"SHORT",EnumToString(InpSignalTF),_Digits,r[0].close,hi,lo,atr);
      PrintFormat("HTF signal %s %s bar=%s close=%.*f chanHi=%.5f chanLo=%.5f atr=%.5f",
         dir>0?"LONG":"SHORT",EnumToString(InpSignalTF),TimeToString(r[0].time,TIME_MINUTES),_Digits,r[0].close,hi,lo,atr);
   }
}
int GetPositionDir()
{
...
// Entry-gap guard: current price must sit near the signal close, else the move gapped away.
   ...
   double gap=MathAbs((g_pendDir>0?q.bid:q.ask)-close);
   if(InpMaxEntryAirGapFrac>0 ? gap>InpMaxEntryAirGapFrac*atr : gap>InpMaxEntryGapPips*pip)
   {
      g_rejectReason=StringFormat("entry air gap %.2f pts (limit %.2f pts)",
         gap,InpMaxEntryAirGapFrac>0?InpMaxEntryAirGapFrac*atr:InpMaxEntryGapPips*pip);
      Reject(); return;
   }
...
// Size from initial-stop risk exactly like the account risk model.
   ...
   bool sent=g_pendDir>0?trade.Buy(lots,_Symbol,0,sl,0,"IND"):trade.Sell(lots,_Symbol,0,sl,0,"IND");
   if(!sent || !ResultOk())
   {
...
void Display()
{
   Comment("IndexBreakout | ",_Symbol,"\n",g_status,
      StringFormat("\nParams: chan=%d ATR=%d stop=%.1fATR trail=%.1fATR risk=%.2f%% airGap=%.2f sess=%d-%d",
         InpChannelBars,InpATRPeriod,InpStopAtr,InpTrailAtr,InpRiskPercent,InpMaxEntryAirGapFrac,InpSessionStart,InpSessionEnd),
      StringFormat("\nSignals %d | Entries %d | Closes %d | Rejects %d | Trail mods %d | Min-lot blocks %d | Out-of-session %d",
         g_signals,g_entries,g_closes,g_rejects,g_trailMods,g_minlotRejects,g_outOfSession),
...
int OnInit()
{
   if(InpChannelBars<2 || InpATRPeriod<2 || InpStopAtr<=0 || InpTrailAtr<=0 || InpRiskPercent<=0 || InpRiskPercent>100
      ...
      || InpMaxEntryAirGapFrac<0 || InpSessionStart<0 || InpSessionStart>23 || InpSessionEnd<0 || InpSessionEnd>24
      || (InpSessionStart!=InpSessionEnd && InpSessionEnd<=InpSessionStart))
   ...
   PrintFormat("IndexBreakout initialized %s (sig %s, chan=%d, ATR=%d, stop=%.1f, trail=%.1f, risk=%.2f%%, airGap=%.2f, sess=%d-%d)",
      _Symbol,EnumToString(InpSignalTF),InpChannelBars,InpATRPeriod,InpStopAtr,InpTrailAtr,InpRiskPercent,InpMaxEntryAirGapFrac,InpSessionStart,InpSessionEnd);
   return INIT_SUCCEEDED;
}
void OnDeinit(const int reason)
{
   PrintFormat("IndexBreakout summary %s: signals=%d, entries=%d, closes=%d, trailMods=%d, rejects=%d, minlotBlocks=%d, outOfSession=%d",
      _Symbol,g_signals,g_entries,g_closes,g_trailMods,g_rejects,g_minlotRejects,g_outOfSession);
